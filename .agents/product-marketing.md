...
| **Free tier = everything with no `EntitlementGate`.** Only: daily recovery score, sleep score + full breakdown, recovery trend, Home/Lock Screen widgets. Never list a Premium feature as free. | `EntitlementGate.gates(for: .free)` returns `[]`; nothing gates recovery or sleep. | `Core/Subscriptions/EntitlementGate.swift:340-343` |
